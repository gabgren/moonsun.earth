...
(() => {
  ...
  // Let app.js re-trace when it jumps to a new instant (Apply time, bookmarks).
  window.recomputeEclipsePath = () => { if (chk.checked) drawPath(viewer.clock.currentTime); };

  // This script loads after app.js, so a restored state (autosave or bookmark)
  // may have ticked the box before the listener above existed — honour it.
  if (chk.checked) drawPath(viewer.clock.currentTime);
})();
